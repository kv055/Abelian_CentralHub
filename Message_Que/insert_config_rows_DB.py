...
class Insert_config_rows_into_Q:
    ...
    def construct_deploy_message(self, config_row):
    ...
    def construct_stop_trading_message(self, row_id):
    ...
    def insert_stop_trading_messages_into_q(self):
        list_of_messages_to_insert_into_q = []

        if len(self.stop_trading) > 0:
            for row in self.stop_trading:
                message_body = self.construct_stop_trading_message(row['id'])
